		...
		this.Step_Access_Model = function( step, opts ) {
			...
			var Model = Backbone.Model.extend( {
				...
				initialize: function() {
					 ...
					 *
					 * - Previous step lock state changes.
					 ...
					 */
					if ( composite.settings.layout !== 'single' && ( composite.settings.layout_variation !== 'componentized' || composite.settings.sequential_componentized_progress === 'yes' && false === self.is_review() ) ) {
						...
						$.each( composite.get_steps(), function( index, step ) {
							...
							if ( step.step_index < self.step_index ) {
								...
								view.listenTo( step.step_validation_model, 'change:passes_validation', view.update_lock_state );
								// Update lock state when the lock state of a previous step changes.
								view.listenTo( step.step_access_model, 'change:is_locked', view.update_lock_state );
							}
				...
				update_lock_state: function() {
					...
					$.each( composite.get_steps(), function( index, step ) {
						...
						if ( step.step_access_model.get( 'is_locked' ) && step.is_visible() ) {
							lock = true;
							return false;
		...
		this.Composite_Price_View = function( opts ) {
			...
			var View = Backbone.View.extend( {
				...
				render_task: function( model ) {
					...
					if ( model.get( 'passes_validation' ) && model.price_data[ 'total' ] !== model.price_data[ 'base_display_price' ] ) {
			...
			var View = Backbone.View.extend( {
				...
				render_indexes: function( after_index ) {
					...
					var summary_element_columns = parseInt( composite.$composite_summary.data( 'columns' ) ),
						summary_element_loop    = after_index,
						view                    = this;
				...
				render_element_content_task: function( step ) {
					...
					if ( step.is_component() ) {
						...
						if ( image_data ) {
							...
							content_data.element_image_srcset = image_data.image_srcset;
							content_data.element_image_sizes  = image_data.image_sizes;
		...
		this.Component_Options_View = function( component, opts ) {
			...
			var View = Backbone.View.extend( {
				...
				render: function( dropdown_only ) {
					...
					composite.actions.add_action( 'options_state_render', [ self ] );
					...
					self.$component_options_select.html( this.templates.dropdown( dropdown_options_data ) );
					...
					if ( false === dropdown_only ) {
						...
						if ( self.has_options_style( 'thumbnails' ) ) {
							...
							if ( new_template_html.replace( / selected/g, '' ) !== this.template_html.replace( / selected/g, '' ) ) {
								this.template_html = new_template_html;
								self.$component_thumbnail_options.html( new_template_html );
							} else {
								composite.console_log( 'debug:views', '...skipped!' );
							}
						...
						// Radio buttons template.
							...
							self.$component_radio_button_options.html( this.templates.radios( radio_options_data ) );
						}
		...
		this.Component_Selection_View = function( component, opts ) {
			...
			var	View = Backbone.View.extend( {
				...
				initialize: function() {
					 ...
					 * remember the position of the relocated container if the 'relocated_content_reset_on_return' flag is set to 'no'.
					 */
					if ( wc_composite_params.relocated_content_reset_on_return === 'no' ) {
				...
				/**
				 * Remember the position of the relocated container when the active step changes:
				 */
				options_state_rendered_handler: function( step ) {
						...
						if ( view.relocate_component_content ) {
							...
							if ( view.relocate_to_origin ) {
								...
								if ( do_illusion_scroll ) {
									...
									var illusion_scroll_offset = view.load_height,
									...
									setTimeout( function() {
										...
										window.scrollBy( 0, - Math.floor( illusion_scroll_offset ) );
			...
			this.validate = function() {
				...
				if ( this.is_component() ) {
					...
					if ( product_id > 0 ) {
						...
						if ( product_type === 'variable' && ( variation_id > 0 || 0 === this.get_selected_quantity() ) ) {
							valid = true;
				...
				if ( param === 'id' ) {
					...
					if ( variation_id > 0 && variations ) {
						$.each( variations, function( index, variation ) {
							if ( parseInt( variation.variation_id ) === parseInt( variation_id ) && variation.image_src ) {
								vi_data      = {
