...
class WC_CP_Install {
	...
	private static function update() {
		...
		foreach ( self::$db_updates as $version => $update_callbacks ) {
			if ( version_compare( self::$current_db_version, $version, '<' ) ) {
				$logger->add( 'wc_cp_db_updates', sprintf( 'Updating to version %s.', $version ) );
				foreach ( $update_callbacks as $update_callback ) {
					$logger->add( 'wc_cp_db_updates', sprintf( '- Queuing %s callback.', $update_callback ) );
	...
	public static function update_complete() {
		...
		$logger->add( 'wc_cp_db_updates', 'Data update complete.' );
		self::update_db_version();
		delete_option( 'wc_cp_update_init' );
	}
	...
	private static function update_db_version( $version = null ) {
		...
		$logger->add( 'wc_cp_db_updates', sprintf( 'Database version is %s.', get_option( 'woocommerce_composite_products_db_version', 'unknown' ) ) );
	}
	...
	public static function plugin_row_meta( $links, $file ) {
		...
		if ( $file == WC_CP()->plugin_basename() ) {
			$row_meta = array(
				...
				'support' => '<a href="https://support.woocommerce.com/">' . __( 'Support', 'woocommerce-composite-products' ) . '</a>',
			);
