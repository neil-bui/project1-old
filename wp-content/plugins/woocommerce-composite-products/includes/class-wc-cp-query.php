...
class WC_CP_Query {
	...
	private function query( $component_data, $query_args ) {
		...
		if ( $args[ 'use_transients_cache' ] ) {
			...
			$transient_name       = 'wc_cp_query_results_' . $component_data[ 'composite_id' ] . '_' . WC_Cache_Helper::get_transient_version( 'product' );
			$cached_results_array = get_transient( $transient_name );

			if ( false !== $cached_results_array && is_array( $cached_results_array ) && isset( $cached_results_array[ $component_id ] ) && is_array( $cached_results_array[ $component_id ] ) ) {
			...
			if ( false === $cached_results ) {
				...
				if ( is_array( $cached_results_array ) ) {
					$cached_results_array[ $component_id ] = $this->results;
				} else {
					$cached_results_array = array( $component_id => $this->results );
				}

				set_transient( $transient_name, $cached_results_array, ( 60 * 60 * 24 ) );
