...
class WC_CP_Admin_Notices {
	...
	public static function updating_notice() {
		...
		if ( WC_CP_Install::is_update_pending() ) {
			...
			if ( gmdate( 'U' ) - get_option( 'wc_cp_update_init', 0 ) > 5 ) {
				...
				$fallback_prompt = '<a href="' . $fallback_url . '">' . __( 'run the update process manually', 'woocommerce-composite-products' ) . '</a>';
				$fallback        = '<br/><em>' . sprintf( __( '&hellip;Taking a while? You may need to %s.', 'woocommerce-composite-products' ), $fallback_prompt ) . '</em>';
				$fallback        = WC_CP_Install::is_update_process_running() ? '' : $fallback;
			}
			$notice = '<strong>' . __( 'WooCommerce Composite Products Data Update', 'woocommerce-composite-products' ) . '</strong> &#8211; ' .  __( 'Your database is being updated in the background.', 'woocommerce-composite-products' ) . $fallback;
