...
function wc_cp_update_370_main( $updater ) {
	...
	if ( ! empty( $composites ) ) {
		foreach ( $composites as $index => $composite ) {
			...
			if ( ! empty( $composite_data ) ) {
				...
				$terms        = get_the_terms( $composite_id, 'product_type' );
...
function wc_cp_update_380_main( $updater ) {
	...
	if ( $composite_term ) {
		...
		if ( ! empty( $composites ) ) {
			foreach ( $composites as $index => $composite ) {
				...
				update_post_meta( $composite_id, '_bto_base_price', false !== $price ? $price : '' );
				delete_post_meta( $composite_id, '_base_price' );

				update_post_meta( $composite_id, '_bto_base_regular_price', false !== $regular_price ? $regular_price : '' );
				delete_post_meta( $composite_id, '_base_regular_price' );

				update_post_meta( $composite_id, '_bto_base_sale_price', false !== $sale_price ? $sale_price : '' );
