...
class WC_CP_Component_View {
	...
	private function get_options_in_scenarios( $options, $scenarios ) {
		...
		$scenario_meta          = $this->component->get_composite()->get_scenario_meta();
		$filtered_scenario_meta = array_intersect_key( $scenario_meta, array_flip( $scenarios ) );

		$component_id   = $this->component->get_id();
		$component_data = $this->component->get_data();

		$component_data[ 'current_component_options' ] = $options;

		$scenario_data = WC_CP_Scenarios::build_scenarios( $filtered_scenario_meta, array( $component_id => $component_data ) );

		foreach ( $scenario_data[ 'scenario_data' ][ $component_id ] as $product_id => $product_in_scenarios ) {
