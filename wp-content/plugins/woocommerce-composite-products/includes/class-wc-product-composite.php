...
class WC_Product_Composite extends WC_Product {
	...
	public function __construct( $bundle_id ) {
		...
		$this->min_raw_regular_price = $this->min_composite_regular_price = get_post_meta( $this->id, '_regular_price', true );
		$this->max_raw_price         = $this->max_composite_price         = get_post_meta( $this->id, '_wc_sw_max_price', true );
		$this->max_raw_regular_price = $this->max_composite_regular_price = get_post_meta( $this->id, '_wc_sw_max_regular_price', true );
	...
	public function sync_composite() {
		...
		$has_finite_max_price = $this->is_nyp() ? false : true;
		...
		if ( $this->contains( 'priced_individually' ) ) {
			...
			if ( ! $this->hide_price_html() ) {
				...
				foreach ( $components as $component_id => $component ) {
					...
					foreach ( $component_options as $id ) {

						$composited_product = $this->get_component_option( $component_id, $id );
					...
					$min_raw_price         = $min_raw_price + $quantity_min * $item_min_price;
					$min_raw_regular_price = $min_raw_regular_price + $quantity_min * $item_min_regular_price;
					...
					if ( $has_finite_max_price ) {
						$max_raw_price         = $max_raw_price + $quantity_max * $item_max_price;
						$max_raw_regular_price = $max_raw_regular_price + $quantity_max * $item_max_regular_price;
					}
				}

			// FAST calculation. When the "Hide Price" option is checked, the expensive loop does not run and all required variables are set via filters. Recommended if your Composite includes more than ~100 products in total.
			} else {
				...
				$min_raw_regular_price = apply_filters( 'woocommerce_min_composite_regular_price', $min_raw_regular_price, $this );
				$max_raw_regular_price = apply_filters( 'woocommerce_max_composite_regular_price', $max_raw_regular_price, $this );
		...
		$this->min_composite_regular_price = $min_raw_regular_price;
		$this->max_composite_price         = $max_raw_price;
		$this->max_composite_regular_price = $max_raw_regular_price;
			...
			if ( isset( $this->composite_price_cache[ $cache_key ] ) ) {
				...
				if ( $this->$prop === '' ) {
					...
					foreach ( $this->price_index[ 'price' ][ $min_or_max ] as $component_id => $product_id ) {
						...
						$item_qty  = $component->is_optional() && $min_or_max === 'min' ? 0 : $component->get_quantity( $min_or_max );
						if ( $item_qty ) {
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->contains( 'priced_individually' ) && ! empty( $components ) ) {
			...
			// Get the price.
			if ( ( $this->hide_price_html() && empty( $this->price_index[ 'price' ][ 'min' ] ) ) || $this->get_composite_price( 'min' ) === '' ) {
	...
	*/
	...
	public static function get_layout_description( $layout_id ) {
		...
		$tooltips = array(
			'single'              => __( 'Components are vertically stacked, with the add-to-cart button located at the bottom.', 'woocommerce-composite-products' ),
			'progressive'         => __( 'Components are vertically stacked and wrapped in toggle-boxes. They must be configured in sequence. Only one Component is visible at a time.', 'woocommerce-composite-products' ),
			'paged'               => __( 'Components are viewed individually and configured in a step-by-step manner. Selections are summarized in a final Review step.', 'woocommerce-composite-products' ),
			'paged-componentized' => __( 'Components are viewed individually and can be configured in any sequence. A variation of the Stepped layout that begins with a configuration Summary.', 'woocommerce-composite-products' ),
		);
	...
	*/
	...
	public function get_scenario_meta() {
		...
		if ( empty( $composite_scenario_meta ) ) {
			$composite_scenario_meta = array();
		}
		 ...
		 * @param  WC_Product_Composite  $product
		 */
		return apply_filters( 'woocommerce_composite_scenario_meta', $composite_scenario_meta, $this );
	}
	...
	public function get_current_scenario_data( $component_ids = array() ) {
		...
		$composite_scenario_meta      = $this->get_scenario_meta();
		$components                   = $this->get_components();
		...
		foreach ( $components as $component_id => $component ) {
			...
			if ( empty( $component_ids ) || in_array( $component_id, $component_ids ) ) {
				...
				$composite_data_for_scenarios[ $component_id ] = $component->get_data();
	...
	*/
	...
	public function get_component( $component_id ) {
		...
		if ( $this->has_component( $component_id ) ) {
			...
			if ( null === $component = WC_CP_Helpers::cache_get( 'wc_cp_component_' . $component_id . '_' . $this->id ) ) {
