...
class WC_CP_Component implements ArrayAccess {
	...
	public function get_options() {
		if ( ! isset( $this->options ) ) {
			$this->options = self::query_component_options( $this->get_data() );
		}
		return $this->options;
	}
	...
	public function get_option( $id ) {
		...
		if ( isset( $this->products[ $id ] ) ) {
			$option_obj = $this->products[ $id ];
		} else {
			$option_obj = new WC_CP_Product( $id, $this->id, $this->composite );
			if ( $option_obj->exists() ) {
				$this->products[ $id ] = $option_obj;
			}
		}
