...
/**
 * Adds hooks for Product Addons and NYP Compatibility.
 *
 * @version  3.8.0
 */
class WC_CP_Addons_Compatibility {
	...
	public static function component_addons_disable( $id, $data, $product_id ) {
		...
		<div class="component_selection_details_option">
			...
			<?php echo WC_CP_Core_Compatibility::wc_help_tip( __( 'Check this option to disable any Product Add-ons associated with the selected Component Option.', 'woocommerce-composite-products' ) ); ?>
		</div>
		<?php
	}
	...
	public static function nyp_display_support( $product, $component_id, $composite ) {
		...
		if ( function_exists( 'WC_Name_Your_Price' ) && ( $product->get_type() === 'simple' || $product->get_type() === 'bundle' ) ) {
	...
	/**
	 * Add some contextual info to Addons/NYP validation messages.
	 *
	 * @param  string $message
	 * @return string
	 */
	public static function component_nyp_and_addons_error_message_context( $message ) {
	...
	public static function validate_component_nyp_and_addons( $add, $composite_id, $component_id, $product_id, $quantity, $cart_item_data, $composite ) {
		...
		if ( ! empty( $Product_Addon_Cart ) ) {
			...
			add_filter( 'woocommerce_add_error', array( __CLASS__, 'component_nyp_and_addons_error_message_context' ) );
			...
			remove_filter( 'woocommerce_add_error', array( __CLASS__, 'component_nyp_and_addons_error_message_context' ) );
		...
		if ( function_exists( 'WC_Name_Your_Price' ) ) {
			...
			if ( ! $composited_product->is_priced_individually() ) {
				return $add;
			}
