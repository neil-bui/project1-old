 ...
 * @return mixed
 */
function wc_cp_get_composited_order_item_container( $maybe_composited_order_item, $order, $return_id = false ) {
 ...
 * @return mixed
 */
function wc_cp_get_composited_order_items( $maybe_composite_container_order_item, $order, $return_ids = false, $deep_mode = false ) {

	$composited_order_items = array();

	if ( wc_cp_is_composite_container_order_item( $maybe_composite_container_order_item ) ) {

		$composited_cart_keys = unserialize( $maybe_composite_container_order_item[ 'composite_children' ] );
 ...
 * @return boolean
 */
function wc_cp_is_composited_order_item( $maybe_composited_order_item, $order ) {
...
function is_composite_product() {
	...
	return function_exists( 'is_product' ) && is_product() && ! empty( $product ) && is_callable( array( $product, 'get_type' ) ) && $product->get_type() === 'composite';
}
...
function wc_cp_price_decimal_sep() {
	...
	if ( null === $wc_price_decimal_sep ) {
		...
		WC_CP_Helpers::cache_set( 'wc_price_decimal_sep', $wc_price_decimal_sep );
	}
...
function wc_cp_price_thousand_sep() {
	...
	if ( null === $wc_price_thousand_sep ) {
		...
		WC_CP_Helpers::cache_set( 'wc_price_thousand_sep', $wc_price_thousand_sep );
	}
...
function wc_cp_price_num_decimals() {
	...
	if ( null === $wc_price_num_decimals ) {
		...
		WC_CP_Helpers::cache_set( 'wc_price_num_decimals', $wc_price_num_decimals );
	}
