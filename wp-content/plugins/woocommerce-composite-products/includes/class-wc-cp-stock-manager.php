...
class WC_CP_Stock_Manager {
	...
	public function validate_stock() {
		...
		foreach ( $managed_items as $managed_item_id => $managed_item ) {
			...
			if ( 'variation' === $product_data->get_type() && WC_CP_Core_Compatibility::is_wc_version_gte_2_5() ) {
				$configuration = sprintf( _x( ' (%s)', 'suffix', 'woocommerce-composite-products' ), WC_CP_Core_Compatibility::wc_get_formatted_variation( $product_data, true ) );
			}
