...
class WC_CP_Compatibility {
	...
	public function check_required_versions() {
		...
		if ( ! empty( $woocommerce_bundles ) && version_compare( $woocommerce_bundles->version, $this->required[ 'pb' ] ) < 0 ) {
			$notice = sprintf( __( '<strong>WooCommerce Composite Products</strong> is not compatible with the versino of <strong>WooCommerce Product Bundles</strong> found on your system. Please update <strong>WooCommerce Product Bundles</strong> to version <strong>%s</strong> or higher.', 'woocommerce-composite-products' ), $this->required[ 'pb' ] );
