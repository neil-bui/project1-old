	...
	*/
	...
	public function parse_composite_configuration( $composite, $configuration = array(), $strict_mode = false ) {
		...
		foreach ( $components as $component_id => $component ) {
			...
			if ( $parsed_configuration[ $component_id ][ 'product_id' ] > 0 ) {
				...
				$parsed_configuration[ $component_id ][ 'type' ] = $product_type;
			}
		}
	...
	public function validate_composite_configuration( $composite, $composite_quantity, $configuration, $context = 'add-to-cart' ) {
		...
		if ( is_object( $composite ) && 'composite' === $composite->get_type() ) {
			...
			$scenario_meta    = $composite->get_scenario_meta();
			...
			foreach ( $validation_data as $component_id => $component_validation_data ) {
				if ( '0' !== $component_validation_data[ 'product_id' ] ) {
					$composite_data[ $component_id ][ 'current_component_options' ] = array( $component_validation_data[ 'product_id' ] );
				} else {
					$composite_data[ $component_id ][ 'current_component_options' ] = array( '' );
				}
			}
			 ...
			 */
			$scenario_data                 = apply_filters( 'woocommerce_composite_scenario_data', WC_CP_Scenarios::build_scenarios( $scenario_meta, $composite_data ), $scenario_meta, $composite_data, $composite );
			$common_scenarios              = array_values( $scenario_data[ 'scenarios' ] );
			$common_compat_group_scenarios = WC_CP_Scenarios::filter_scenarios_by_type( $common_scenarios, 'compat_group', $scenario_data );

			// Validate Selections.
			foreach ( $validation_data as $component_id => $component_validation_data ) {

				$validate_product_id          = isset( $component_validation_data[ 'variation_id' ] ) ? $component_validation_data[ 'variation_id' ] : $component_validation_data[ 'product_id' ];
				$scenarios_for_product        = array();
				$selection_invalid            = false;
				$conditional_component_hidden = false;

				if ( ! empty( $scenario_data[ 'scenario_data' ][ $component_id ][ $validate_product_id ] ) ) {
					$scenarios_for_product              = $scenario_data[ 'scenario_data' ][ $component_id ][ $validate_product_id ];
					$compat_group_scenarios_for_product = WC_CP_Scenarios::filter_scenarios_by_type( $scenarios_for_product, 'compat_group', $scenario_data );
				}

				if ( empty( $compat_group_scenarios_for_product ) ) {
					$selection_invalid = true;
				}

				if ( '0' === $validate_product_id && 'no' === $component_validation_data[ 'optional' ] ) {

					// Check if the product is part of a hidden/conditional component.
							...
							foreach ( $conditional_components as $scenario_id => $hidden_components ) {
								if ( in_array( $scenario_id, $conditional_components_scenarios ) ) {
									if ( ! empty( $hidden_components ) && in_array( $component_id, $hidden_components ) ) {
										$conditional_component_hidden = true;
										break;
									}
								}
							}
					...
					if ( false === $conditional_component_hidden ) {
						$selection_invalid = true;
					}
				}

				if ( $selection_invalid ) {
					if ( '0' === $validate_product_id ) {
						 ...
						 * @param   string   $composite_id
						 */
						if ( apply_filters( 'woocommerce_composite_validation_component_is_mandatory', true, $component_id, $component_validation_data, $common_scenarios, $scenario_data, $composite_id ) ) {
							if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
								wc_add_notice( sprintf( __( 'Please select a &quot;%s&quot; option.', 'woocommerce-composite-products' ), $component_validation_data[ 'title' ] ), 'error' );
							} elseif ( 'cart' === $context ) {
								wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. A &quot;%2$s&quot; selection is required.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ] ), 'error' );
							}
							return false;
						}
						...
						if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
							wc_add_notice( sprintf( __( 'Please select a different &quot;%s&quot; option &mdash; the selected product cannot be purchased at the moment.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ] ), 'error' );
						} elseif ( 'cart' === $context ) {
							wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased &ndash; the chosen &quot;%2$s&quot; option is unavailable.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ] ), 'error' );
						}
						return false;
					}

				} elseif ( false === $conditional_component_hidden ) {
			...
			if ( empty( $common_compat_group_scenarios ) ) {
				if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
					...
					wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. The selected options cannot be purchased together.', 'woocommerce-composite-products' ), $component_validation_data[ 'title' ] ), 'error' );
				}
				return false;
			}
			...
			foreach ( $validation_data as $component_id => $component_validation_data ) {
				 ...
				 * @param  string  $composite_id
				 */
				$qty_min = absint( apply_filters( 'woocommerce_composite_validation_component_quantity_min', $component_validation_data[ 'quantity_min' ], $component_id, $component_validation_data, $common_scenarios, $scenario_data, $composite_id ) );
				 ...
				 * @param  string  $composite_id
				 */
				$qty_max = absint( apply_filters( 'woocommerce_composite_validation_component_quantity_max', $component_validation_data[ 'quantity_max' ], $component_id, $component_validation_data, $common_scenarios, $scenario_data, $composite_id ) );
	...
	private function set_composited_cart_item_price( $cart_item, $component_id, $composite ) {
		...
		$cart_item[ 'data' ]->price = $component_option->get_raw_price( $cart_item[ 'data' ], 'cart' );

		if ( false === $component_option->is_priced_individually() && false === $cart_item[ 'data' ]->price > 0 ) {
	...
	*/
	...
	public function add_to_cart_validation( $add, $product_id, $quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
		...
		if ( isset( $cart_item_data[ 'is_order_again_composited' ] ) && $order_again ) {
			// ...but let them go through without issues when re-populating the cart to pay for a pending WC Subs renewal order.
			if ( ! isset( $cart_item_data[ 'subscription_resubscribe' ] ) && ! isset( $cart_item_data[ 'subscription_renewal' ] ) ) {
				return false;
			}
		}

		if ( 'composite' === $product_type && $add ) {

			// Get product.
			$composite = wc_get_product( $product_id );

			// Check 'sold_individually' context.
			if ( $composite->is_sold_individually() ) {
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					if ( $product_id === $cart_item[ 'product_id' ] ) {
						...
						if ( isset( $_POST[ 'update-composite' ] ) ) {
							$editing_key = wc_clean( $_POST[ 'update-composite' ] );
							if ( $editing_key === $cart_item_key ) {
						...
						if ( $item_found_in_cart ) {
							if ( 'configuration' === $composite->get_sold_individually_context() ) {
								wc_add_notice( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', WC()->cart->get_cart_url(), __( 'View Cart', 'woocommerce' ), sprintf( __( 'You have already added an identical &quot;%s&quot; to your cart. You cannot add another one.', 'woocommerce-composite-products' ), $composite->get_title() ) ), 'error' );
							} else {
	...
	public function add_items_to_cart( $composite_cart_key, $composite_id, $composite_quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
			...
			foreach ( $cart_item_data[ 'composite_data' ] as $component_id => $component_configuration ) {
				...
				// Get product type.
				$terms                   = get_the_terms( $composited_product_id, 'product_type' );
				$composited_product_type = ! empty( $terms ) && isset( current( $terms )->name ) ? sanitize_title( current( $terms )->name ) : 'simple';
		...
		if ( $composite_container_item_key = wc_cp_get_composited_cart_item_container( $values, WC()->cart->cart_contents, true ) ) {
			...
			if ( $component_option = $composite_container_item[ 'data' ]->get_component_option( $component_id, $product_id ) ) {
				...
				if ( false === $component_option->get_component()->is_subtotal_visible( 'cart' ) || false === $component_option->is_priced_individually() ) {
					...
					if ( apply_filters( 'woocommerce_add_composited_cart_item_subtotals', true, $composite_container_item, $composite_container_item_key ) ) {
						$subtotal = __( 'Option subtotal', 'woocommerce-composite-products' ) . ': ' . $subtotal;
					}
				}
			}
			...
			if ( apply_filters( 'woocommerce_add_composited_cart_item_subtotals', true, $values, $cart_item_key ) ) {
				...
				if ( ! empty( $children ) ) {
					...
					$composite_price        = 'excl' === get_option( 'woocommerce_tax_display_cart' ) ? $values[ 'data' ]->get_price_excluding_tax( $values[ 'quantity' ] ) : $values[ 'data' ]->get_price_including_tax( $values[ 'quantity' ] );

					foreach ( $children as $child_key => $child_data ) {
						$composited_items_price += 'excl' === get_option( 'woocommerce_tax_display_cart' ) ? $child_data[ 'data' ]->get_price_excluding_tax( $child_data[ 'quantity' ] ) : $child_data[ 'data' ]->get_price_including_tax( $child_data[ 'quantity' ] );
					}
						...
						if ( wc_cp_is_composite_container_cart_item( $cart_item_data ) ) {
							...
							if ( $composite->needs_shipping() ) {
								...
								foreach ( wc_cp_get_composited_cart_items( $cart_item_data, WC()->cart->cart_contents, true ) as $child_item_key ) {
									...
									$composited_product     = unserialize( serialize( $child_cart_item_data[ 'data' ] ) );
								...
								if ( isset( $composite->bundled_weight ) ) {
									$composite->bundled_weight += $bundled_weight / $composite_qty;
								}

								if ( isset( $composite->bundled_value ) ) {
									$composite->bundled_value += $bundled_value / $composite_qty;
								}
				...
				if ( apply_filters( 'woocommerce_composite_inherit_coupon_validity', true, $product, $coupon, $cart_item, $container_cart_item ) ) {
					...
					if ( $valid ) {
						...
						if ( sizeof( $coupon->exclude_product_categories ) > 0 ) {
							...
							$product_cats = $parent_id ? WC_CP_Core_Compatibility::wc_get_product_cat_ids( $parent_id ) : WC_CP_Core_Compatibility::wc_get_product_cat_ids( $product_id );
