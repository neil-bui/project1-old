	...
	public static function updating_notice() {
		...
		if ( WC_PB_Install::is_update_pending() ) {
			...
			if ( gmdate( 'U' ) - get_option( 'wc_pb_update_init', 0 ) > 5 ) {
				...
				$fallback_prompt = '<a href="' . $fallback_url . '">' . __( 'run the update process manually', 'woocommerce-product-bundles' ) . '</a>';
				$fallback        = '<br/><em>' . sprintf( __( '&hellip;Taking a while? You may need to %s.', 'woocommerce-product-bundles' ), $fallback_prompt ) . '</em>';
				$fallback        = WC_PB_Install::is_update_process_running() ? '' : $fallback;
			}
			$notice = '<strong>' . __( 'WooCommerce Product Bundles Data Update', 'woocommerce-product-bundles' ) . '</strong> &#8211; ' .  __( 'Your database is being updated in the background.', 'woocommerce-product-bundles' ) . $fallback;
