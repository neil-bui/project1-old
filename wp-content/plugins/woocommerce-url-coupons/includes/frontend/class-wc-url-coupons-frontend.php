...
class WC_URL_Coupons_Frontend {
	...
	public function get_defer_apply_notice_message( $error_message, $error_code, $coupon ) {
		...
		if ( $this->is_error_actionable( $error_code ) ) {
			...
			$deferred_notice      = isset( $this->deferred_coupons[ $coupon->id ] ) ? $coupon_already_added : $coupon_added;
	...
	/**
	 * Determine if provided error code is actionable. i.e. The customer is able
	 * to recover from the error by performing an action such as removing
	 * something from the cart.
	 *
	 * @since 2.4.0
	 * @param int $error_code The error code
	 * @return bool true if error is actionable, false otherwise
	 */
	private function is_error_actionable( $error_code ) {
		 ...
		 */
		return apply_filters( 'wc_url_coupons_is_error_actionable', in_array( $error_code, $actionable_errors ) );
	}
