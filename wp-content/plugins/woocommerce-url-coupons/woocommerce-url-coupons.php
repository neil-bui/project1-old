 ...
 * Plugin URI: http://www.woothemes.com/products/url-coupons/
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce URL Coupons', 'woocommerce-url-coupons' ), __FILE__, 'init_woocommerce_url_coupons', array(
...
class WC_URL_Coupons extends SV_WC_Plugin {
	...
	/** plugin text domain, DEPRECATED as of 2.2.0 */
	const TEXT_DOMAIN = 'woocommerce-url-coupons';
	...
	public function load_translation() {
		...
		load_plugin_textdomain( 'woocommerce-url-coupons', false, dirname( plugin_basename( $this->get_file() ) ) . '/i18n/languages' );
	}
	...
	public function get_documentation_url() {
	    return 'http://docs.woothemes.com/document/url-coupons/';
	}
	...
	public function upgrade( $installed_version ) {
		...
		if ( version_compare( $installed_version, '2.1.1', '<' ) ) {
			...
			foreach ( $coupons as $coupon_id => $data ) {
				...
				// update redirect page type
				if ( empty( $data['redirect'] ) || get_post_meta( $coupon_id, '_wc_url_coupons_redirect_page_type', true ) ) {
					continue;
				}
	...
	public function __get( $property ) {
		...
		$main_class = 'wc_url_coupons()';
		$deprecated = $main_class . '->' . $property;

		switch ( $property ) {

			case 'admin' :
				_deprecated_function( $deprecated, '2.3.0', $main_class . '->get_admin_instance()' );
				return $this->get_admin_instance();

			case 'ajax' :
				_deprecated_function( $deprecated, '2.3.0', $main_class . '->get_ajax_instance()' );
				return $this->get_ajax_instance();

			case 'frontend' :
				_deprecated_function( $deprecated, '2.3.0', $main_class . '->get_frontend_instance()' );
