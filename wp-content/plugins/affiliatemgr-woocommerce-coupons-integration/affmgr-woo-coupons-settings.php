...
function wpam_woo_coupons_admin_interface() {
    echo '<div class="wrap">';
    echo '<div id="poststuff"><div id="post-body">';
    echo '<h2>Affiliates Manager WooCommerce Coupons Addon</h2>';
    ...
    if (isset($_POST['aff_woo_save_association'])) {
        ...
        if (!empty($coupon_code) || !empty($aff_id)) {
            $item = new AFFMGR_WOO_ASSOC_ITEM($coupon_code, $aff_id);
            $collection_obj = AFFMGR_WOO_COUPONS_ASSOC::get_instance();
            $collection_obj->add_item($item);
            AFFMGR_WOO_COUPONS_ASSOC::save_object($collection_obj);
    ...
    <div class="postbox">
        ...
        <div class="inside">
            ...
            <p style="background: #fff6d5; border: 1px solid #d1b655; color: #3f2502; margin: 10px 0;  padding: 5px 5px 5px 10px;">
                Read the <a href="https://wpaffiliatemanager.com/tracking-affiliate-commission-using-woocommerce-coupons-or-discount-codes/" target="_blank">usage documentation</a> to learn how to use the WooCommerce Coupons addon
            </p>
