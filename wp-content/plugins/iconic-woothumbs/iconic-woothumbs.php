...
class Iconic_WooThumbs {
    ...
    public function bulk_edit_page() {
        ...
        add_submenu_page( 'woocommerce', __('Bulk Edit Variation Images', 'iconic-woothumbs'), __('WooThumbs Bulk', 'iconic-woothumbs'), 'manage_woocommerce', $this->bulk_edit_slug, array( $this, 'bulk_edit_page_display' ) );
    ...
    public function get_selected_variation( $current_id ) {
        ...
        if ($product->product_type == 'variable') {
            ...
            $variation = $product->get_matching_variation( $selected_atts );
    ...
    public function get_default_atts( $product_id ) {
        ...
        if( $variable_product->product_type != "variable" )
            return $atts;

        $default_atts = $variable_product->get_variation_default_attributes();
    ...
    public function get_all_image_ids( $id ) {
        ...
        if ( false === ( $all_images = get_transient( $transient_name ) ) ) {
            ...
            if ( $has_featured_image ) {
                ...
                if ( isset( $product->parent->id ) && has_post_thumbnail( $product->parent->id )) {
                    $all_images['featured'] = get_post_thumbnail_id( $product->parent->id );
                } else {
            ...
            if ( $post_type == 'product' || $show_gallery ) {
                ...
                $id = isset( $product->parent->id ) ? $product->parent->id : $id;
                $gallery_product = wc_get_product( $id );

                $attach_ids = $gallery_product->get_gallery_attachment_ids();
    ...
    public function get_all_image_sizes( $variation_id ) {
        ...
        if (!empty($image_ids)) {
            foreach ($image_ids as $image_id):
                ...
                if ( false === ( $image_sizes = get_transient( $transient_name ) ) ) {
                    ...
                    $image_sizes = apply_filters( 'iconic_woothumbs_all_image_sizes', $image_sizes, $image_id );
        ...
        return apply_filters( 'iconic_woothumbs_all_image_sizes', $images, $image_id );
    ...
    public function ajax_get_variation() {
        ...
        if( ( isset( $_GET['variation_id'] ) && !empty( $_GET['variation_id'] ) ) && ( isset( $_GET['product_id'] ) && !empty( $_GET['product_id'] ) && $_GET['product_id'] )  ) {
