...
class WCAP_Templates_Table extends WP_List_Table {
	...
	public function column_template_name( $template_row_info ) {	
	    ...
	    $row_actions['copy']   = '<a href="' . wp_nonce_url( add_query_arg( array( 'action' => 'emailtemplates', 'mode'=>'copytemplate', 'id' => $template_row_info->id ), $this->base_url ), 'abandoned_order_nonce') . '">' . __( 'Duplicate', 'woocommerce-ac' ) . '</a>';
	...
	/***
	 * This function used to get the abandoned orders count
	 */
    public function wcap_get_templates_count() {	
	    ...
	    $query             = "SELECT * FROM `" . $wpdb->prefix."ac_email_templates`";
	...
	public function wcap_templates_data() { 
    	...
    	$query                 = "SELECT wpet . * FROM `" . $wpdb->prefix . "ac_email_templates` AS wpet ORDER BY day_or_hour desc , frequency asc";
        $results               = $wpdb->get_results( $query );		
    	...
    	foreach( $results as $key => $value ) {    	    
    	    ...
    	    $id                          = $value->id;
    	    $query_no_emails             = "SELECT * FROM " . $wpdb->prefix . "ac_sent_history WHERE template_id= %d";
    	    $number_emails               = $wpdb->get_results( $wpdb->prepare( $query_no_emails, $id ) );
    	    $ac_emails_count	         = count( $number_emails );
    	    $query_no_recovers           = "SELECT template_id FROM " . $wpdb->prefix . "ac_sent_history WHERE recovered_order= '1'";
    	    $recover_orders		         = $wpdb->get_results( $query_no_recovers );
    	    $temp_id_array		         = array ( );
    	    $recovered_count	         = "0";
    	    
    	    foreach( $recover_orders as $recover_key => $recover_value ) {    	
    	        $template_id     = $recover_value->template_id;
    	        $temp_id_array[] = $template_id;
    	    }
    	    $recover_ord         = array_count_values( $temp_id_array );
    	    foreach( $recover_ord as $k => $v ) {
    	        if( array_key_exists( $k, $recover_ord ) ){
    	            if( $id == $k ) {
    	                $recovered_count = $v;
    	            }
    	        }
    	        if( ! array_key_exists( $id, $recover_ord ) ) {
    	            $recovered_count = "0";
    	        }
    	    }
    	    ...
    	    if ( $ac_emails_count != 0 ) {
    	        $val = $recovered_count/count( $number_emails ) * 100;
    	    } else {
    	        $val = "0";
    	    }
    	    ...
    	    $return_templates_data[ $i ]->percentage_recovery  = round ( $val , 2)."%";
	...
	public function column_default( $wcap_abandoned_orders, $column_name ) {
	    ...
	    switch ( $column_name ) {	       
	        case 'sr' :
	            if( isset( $wcap_abandoned_orders->sr ) ) {
	                $value = $wcap_abandoned_orders->sr;
	            }
	            break;	            
			case 'template_name' :
			    if( isset( $wcap_abandoned_orders->template_name ) ) {
				    $value = $wcap_abandoned_orders->template_name;
			    }
				break;			
			case 'sent_time' :
			    if( isset( $wcap_abandoned_orders->sent_time ) ) {
			       $value = $wcap_abandoned_orders->sent_time;
			    }
				break;			
			case 'activate' :
			    if( isset( $wcap_abandoned_orders->activate ) ) {			       
			       $active    = $wcap_abandoned_orders->activate;
			       ...
			       $value   = '<a href="#" onclick="wcap_activate_email_template('. $id.', '.$is_active.' )"> '.$active_text.'</a>'; 
			       //$value   = $wcap_abandoned_orders->activate;
			    }
				break;			
			case 'email_sent' :
				...
				break;			
			case 'percentage_recovery' :
			    if( isset( $wcap_abandoned_orders->percentage_recovery ) ) {
			     $value = $wcap_abandoned_orders->percentage_recovery;
			    }
			    break;
		    default:			    
