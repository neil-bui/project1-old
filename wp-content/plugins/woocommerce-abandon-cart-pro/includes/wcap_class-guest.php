<?php 
/* 
 * Woocommerce Abandoned Cart Plugin - Saves guest cart information  
*/
if ( ! class_exists( 'woocommerce_guest_ac' ) ) {
	...
	function save_data() {		
        if ( ! is_user_logged_in() ) { 
            if( session_id() === '' ) {   
                session_start(); //session has not started
            }
            global $wpdb, $woocommerce; 
            // If a record is present in the guest cart history table for the same email id, then delete the previous records
            $query_existing_guest   = "SELECT * FROM `" . $wpdb->prefix . "ac_guest_abandoned_cart_history` 
	                                      WHERE email_id = %s AND billing_first_name = %s AND billing_last_name = %s";
            $results_existing_guest = $wpdb->get_results( $wpdb->prepare( $query_existing_guest, $_POST['billing_email'], $_POST['billing_first_name'], $_POST['billing_last_name'] ) ); 
            
            if ( isset( $results_existing_guest ) && count ( $results_existing_guest ) > 0 && isset( $_POST['billing_email'] ) && $_POST['billing_email'] != '') {   
               $update_mobile_info = "UPDATE `" . $wpdb->prefix . "ac_guest_abandoned_cart_history` SET phone = %s 
                                        WHERE email_id = %s AND billing_first_name = %s AND billing_last_name = %s";
               $wpdb->query( $wpdb->prepare( $update_mobile_info, $_POST['billing_phone'],$_POST['billing_email'], $_POST['billing_first_name'], $_POST['billing_last_name'] ) ); 

               $_SESSION['billing_phone'] = $_POST['billing_phone'];
               if ( isset( $_SESSION['user_id'] ) ) {
                   $guest_id = $_SESSION['user_id'];
               }
               $query_update_get    = "SELECT * FROM `" . $wpdb->prefix . "ac_abandoned_cart_history` 
               ...
               $_SESSION['abandoned_cart_id'] = $abandoned_cart_id;                  
                ...
                if ( $results_guest ) {                    
                    foreach( $results_guest as $key => $value ) {
                        ...
                        if ( $result ) {
                            $query_update_same_record = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET cart_ignored = '1'
                ...
                if ( isset( $_SESSION['billing_first_name'] ) && $_SESSION['billing_first_name'] != '' ) {
                    $billing_first_name = $_SESSION['billing_first_name'];
                } else {
                    $billing_first_name = '';
                }
                if ( isset( $_SESSION['billing_last_name'] ) && $_SESSION['billing_last_name'] != '' ){
                    $billing_last_name = $_SESSION['billing_last_name'];
                } else {
                    $billing_last_name = '';
                }
                if ( isset( $_SESSION['billing_phone'] ) && $_SESSION['billing_phone'] != '' ) {
                ...
                } else if ( isset( $_SESSION['billing_postcode'] ) && $_SESSION['billing_postcode'] != "" ) {
                    $shipping_zipcode = $billing_zipcode = $_SESSION['billing_postcode'];
                }   			
                $shipping_charges = $woocommerce->cart->shipping_total;	
                		
                $insert_guest     = "INSERT INTO `" . $wpdb->prefix . "ac_guest_abandoned_cart_history`( billing_first_name, billing_last_name, email_id, phone, billing_zipcode, shipping_zipcode, shipping_charges ) 
                                    VALUES ( '" . $billing_first_name . "', '" . $billing_last_name . "', '" . $_SESSION['billing_email'] . "', '" . $billing_phone . "', '" . $billing_zipcode . "', '" . $shipping_zipcode . "', '" . $shipping_charges . "' )";
                ...
                if ( count( $results ) == 0 ) {                
                    ...
                    if ( count( $results ) == 0 ) {                    
                        $insert_query = "INSERT INTO `" . $wpdb->prefix . "ac_abandoned_cart_history`( user_id, abandoned_cart_info, abandoned_cart_time, cart_ignored, recovered_cart, user_type, language )
                                        VALUES ( '" . $user_id . "', '" . $cart_info . "', '" . $current_time . "', '0', '0', 'GUEST', '". $current_user_lang ."' )";	
                        ...
                        $query_update        = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET user_id = '" . $user_id . "', abandoned_cart_info = '" . $cart_info . "', abandoned_cart_time  = '" . $current_time . "', language = '". $current_user_lang ."'  WHERE session_id ='" . $get_cookie[0] . "' AND cart_ignored='0' ";
